...
-- 5. Clients sans aucune transaction
SELECT id_client, nom, prenom
FROM CLIENT
WHERE id_client NOT IN (
    SELECT DISTINCT te.id_client
    FROM TENANCE te
    JOIN CREDITE c ON te.id_compte = c.id_compte
    JOIN TRANSACTION t ON c.id_transaction = t.id_transaction
);
